...
import de.thb.pizzapronto.controller.IOrdering;
...
public class Ordering implements IOrdering {
	...
	public void confirmOrder() throws NoOrderException, NoCustomerException, IllegalStateException {
		...
		if (currentOrder.getState() == StateOfOrderVO.STARTED) {
			...
			try { 
				startService();
			} catch(IllegalStateException | NoCustomerException | NoOrderException | NotEnoughTimeException e) {
	...
	private void startService() throws NoOrderException, NoCustomerException, IllegalStateException, NotEnoughTimeException {
		if (currentOrder == null) {
			throw new NoOrderException("There is no order.");
		}
